...
GameState::GameState(StateManager* manager, Platform *platform, DataManager *dtmng, RandMap *map, int mode)
	: State(manager, platform), dtmng(dtmng), mode(mode)
{
	stateName = "GameState";
...
void GameState::update(float dt)
{
	...
	if (changeFloor)
	{
		if (mode == 0)
		{
			RandMap *newFloor = new RandMap(dtmng->getMapManager(), dtmng->getTileTypeManager(), dtmng->getCreatureManager(), 22);
			RandMap *oldFloor;
	...
	for (int i = 0; i < randFloor->getCurMap()->getNumCreatures(); i++)
	{
		...
		if (player->getHealth() <= 0)
		{
			if (mode == 0)
			{
				stateManager->changeState(new GameOverState(stateManager, platform, dtmng));
			}
			else if (mode == 1) {
				stateManager->popLastState();
			}
...
void GameState::load()
{
	...
	playerType = dtmng->getCreatureManager()->getCharacterType("P0");
